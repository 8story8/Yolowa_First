...
if (window.jQuery)(function ($) {
	...
	$.fn.MultiFile = function (options) {
		...
		this
			...
			.each(function () {
				...
				// HTML5: enforce multiple selection to be enabled, except when
				// explicitly disabled
				if (o.multiple !== false) {
					...
					trigger: function (event, element, MultiFile, files) {
						...
						if (handler){
							...
							$.each(files,function(i, file){
								// execute function in element's context, so
								// 'this' variable is current element
				...
				MultiFile.addSlave = function (slave, slave_count) {
					...
					// CHANGE v2.2.1 - change ID of all file elements, keep
					// original ID in wrapper
					...
					slave.change(function (a, b, c) {
						...
						$.each(files, function (i, file) {
							...
							$(MultiFile.wrapper).find('input[type=file]').not(e).each(function(){
								...
								$.each(FILE_LIST(this), function (i, file) {
									if(file.name){
										// console.log('MultiFile.debug>
										// Duplicate?', file.name, v);
						...
						if (ERROR.length > 0) {
							...
							// 2007-06-24: BUG FIX - Thanks to Adrian Wróbel
				...
				MultiFile.addToList = function (slave, slave_count, files) {
					// if(window.console)
					// console.log('MultiFile.addToList',slave_count);
					...
					$.each(files, function (i, file) {
						...
						// just add an <img class='MultiFile-preview'/> anywhere
						// within the "file" string
					...
					// $.each(files, function (i, file) {
						...
						var
							...
							b = $('<a class="MultiFile-remove" href="#' + MultiFile.wrapID + '">' + MultiFile.STRING.remove + '</a>')
								...
								// ********
								// TODO:
								// refactor this as a single event listener on
								// the control's
								// wrapper for better performance and cleaner
								// code
								// ********
								.click(function () {
									...
									// remove the relevant <input type="file"/>
									// element
									$(slave).remove();
									...
									// point to currently visible element
									// (always true, not necessary)
									// MultiFile.current =
									// MultiFile.wrapper.find('[type=file]:visible');
									...
									// update current control's reference to
									// MultiFile object
									$(MultiFile.wrapper).data('MultiFile', MultiFile);
				...
				// deprecated: contorl's data now stored in wrapper because it
				// is never removed.
	...
	$.extend($.fn.MultiFile, {
		 ...
		 * 
		 * Returns an object with various settings and properties of the
		 * selected files for this particular instance of the control. stored in
	...
	$.fn.MultiFile.options = { // $.extend($.fn.MultiFile, { options: {
		accept: 'jpg|png|gif', // accepted file extensions
		max: 3, // maximum number of selectable files
		maxfile: 1024, // maximum size of a single file
		maxsize: 3024, // maximum size of entire payload
