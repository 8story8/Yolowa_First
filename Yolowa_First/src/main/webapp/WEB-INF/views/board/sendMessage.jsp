	...
	$(document).ready(
			function() {
				$("#trashMsg").click(function() {
					...
					location.href="${pageContext.request.contextPath}/deleteMessage.do?deleteMsg="+checkboxValues+"&type=send&myId=${sessionScope.member.id}";
				});
				...
				$("#receiveMessage tr td").on("click", function() {
				...
				$("#sendMsgBtn").click(
						function() {
							...
							var sId = "${sessionScope.member.id}"; // 보내는 Id
...
<div class="col-md-12 mail-wrapper">
	<div class="col-md-12 padding-0">
		...
		<div class="col-md-9 mail-right">
			...
			<c:set value="${requestScope.mySendMsg.myAllMsgList.messageList}"
				var="msgList"></c:set>
			<c:set value="${requestScope.mySendMsg.myAllMsgList.pagingBean}"
				var="page"></c:set>
			<c:set value="${requestScope.mySendMsg.memberInfo}" var="sendMember"></c:set>
			<div class="col-md-12 mail-right-content">
				<table class="table table-hover">
					...
					<tbody id="receiveMessage">
						<c:forEach items="${msgList }" var="m">
							<c:forEach items="${sendMember }" var="rId">
								<c:choose>
									<c:when test="${m.mCheck=='SM' or m.mCheck=='RD'}">
										<tr class="read">
											<c:if test="${rId.id == m.rId }">
												<td class="check"><input type="checkbox" class="icheck"
													name="checkbox1" value="${m.mNo }"></td>
												<td class="contact">${rId.name }(${m.rId })</td>
				...
				<ul class="pagination pagination-lg">
					<c:if test="${page.previousPageGroup }">
						<li><a
							href="sendMessagePage.do?curPage=${page.startPageOfPageGroup-1}">◀</a></li>
					</c:if>
					<c:forEach begin="${page.startPageOfPageGroup }"
						...
						<c:choose>
							...
							<c:otherwise>
								<li><a href="sendMessagePage.do?curPage=${i}">${i }</a></li>
							</c:otherwise>
						</c:choose>
					</c:forEach>
					<c:if test="${page.nextPageGroup }">
						<li><a
							href="sendMessagePage.do?curPage=${page.endPageOfPageGroup+1}">▶</a></li>
					</c:if>
				</ul>
			</div>
		</div>
		...
		<div class="modal-dialog">
			...
			<div class="modal-content">
				<div class="modal-header">
					받는 사람 : <span id="sendId"></span><br> 보낸 시간 : <span
